...
const SortType SORT_TYPE = QUICK_SORT;

void sort_cars_data(Car *cars, int count) {
    using namespace std;

    int sort_key;
    sort_key = parse_sort_key();

    switch (sort_key) {
        case BY_MAX_SPEED:
            sort_cars_data_by(cars, count, cmp_cars_by_max_speed);
            break;
        case BY_ENGINE_VOLUME:
            sort_cars_data_by(cars, count, cmp_cars_by_engine_volume);
            break;
        case BY_YEAR:
            sort_cars_data_by(cars, count, cmp_cars_by_year);
            break;
        default:
...
void sort_cars_data_by(Car *cars, int count, int cmp(Car, Car)) {
    switch (SORT_TYPE) {
        case QUICK_SORT:
            quick_sort(cars, 0, count - 1, cmp);
            break;
        case SELECTION_SORT:
            selection_sort(cars, count, cmp);
            break;
    }
}
